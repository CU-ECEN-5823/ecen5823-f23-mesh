...
//      }
    ...
    case sl_bt_evt_system_external_signal_id: {
      ...
      // --------------------------------------
      // This code sends commands to the Server
      // --------------------------------------
...
//          app_log("PB0 Long Pressed, period_idx=%d.\r\n", period_idx);
      ...
      // -----------------------------------------------------------------------
      // Sending/publishing a message to a group address is accomplished by:
      // 1- First setting the data to publish with sl_btmesh_vendor_model_set_publication()
      // 2- And then calling sl_btmesh_vendor_model_publish() to send the actual message
      // -----------------------------------------------------------------------

      // DOS: Set the message data to publish, i.e. queue the data for publishing.

      // Student edit: Uncomment the following code to enable creating the data to publish,
      //               and then actually publish the message/command to the group address.
...
// Students: See comment below on Friendship terminated events
//  static int       friend_term_count = 0; // count of number of lost friendship termination events
    ...
    case sl_btmesh_evt_vendor_model_receive_id: {
      ...
      switch (rx_evt->opcode) {
        ...
        case temperature_status:
          ...
          temperature = *(int32_t *) rx_evt->payload.data; // temperature is in "milli-values"

//          app_log(" ***TEMP value = %d\r\n", temperature); // DOS

          // Display the integer portion of the temperature on the LCD
          if (unit == celsius) {
              snprintf(temperature_string, 10, "Temp=%dC", (int) temperature/1000);
          } else {
              snprintf(temperature_string, 10, "Temp=%dF", (int) temperature/1000);
          }
          sl_btmesh_LCD_write(temperature_string, LCD_ROW_4);
...
//                  unit == celsius ? (char * )"Celsius" : (char * )"Fahrenheit");
    ...
    // Students Note: There is an "issue" with the current code that I have not
    // been able to resolve after months of experimentation. The issue is that
    // after Friendship is established between the Friend node and LPN node,
    // Friendship terminates for some reason. What that reason is is unclear.
    // If you un-comment the lines below, you will see these Friendship
    // terminated/established log messages. I believe this "should be" ok for
    // your course projects as the period of "Friendship not established" is very
    // short, on the order of ~ less than 500 ms in my observations.
